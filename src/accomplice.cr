module Accomplice
  ...
  module Windows
    module Service
      ...
      # Default hint given to the Windows Service Manager for how long
      # starting or stopping the service should take
      DEFAULT_WAIT_HINT = 5.seconds
      # How long to wait before forcibly exiting the program on service
      # shutdown
      @@shutdown_timeout = DEFAULT_SHUTDOWN_TIMEOUT
      # The current Windows Service status
      ...
      # The Windows Service status handle used to update the status with the
      # Windows Service Manager
      @@status_handle : LibWindowsService::SERVICE_STATUS_HANDLE?
      # Mutex to synchronize / serialize changes to the Windows Service status
      @@status_mutex = Mutex.new(Mutex::Protection::Reentrant)
      # Channel used to asynchronously indicate whether we connected to the
      # Windows Service Manager successfully or not
      ...
      # The Windows Service main logic, invoked from a thread created
      ...
      def self.register
        ...
        Fiber::ExecutionContext::Isolated.new("WINDOWS SERVICE DISPATCHER") do
          ...
          if LibWindowsService.StartServiceCtrlDispatcherA(services.to_unsafe) == 0
            ...
            # The service dispatcher has connected successfully to the Windows
            # Service Manager, which will now create a new thread in this
            # process and call the `main` function to start and run the
            # service
          end
        rescue ex
          @@connection_channel.send({false, ex})
        ...
        # service and is responsible for its own handling of signals
        connected, exception = @@connection_channel.receive

        # failed to connect to the Windows Service Manager
        raise exception if exception

        # successfully connected to the Windows Service Manager, and are now
        # running as a service
        if connected
          # register default interrupt signal handler to exit gracefully with
